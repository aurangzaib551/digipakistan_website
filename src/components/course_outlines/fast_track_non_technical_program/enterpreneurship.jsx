import React, { useState } from "react";
import Paper from "@material-ui/core/Paper";
import Button from "@material-ui/core/Button";
import Tabsi from "@material-ui/core/Tabs";
import Tabi from "@material-ui/core/Tab";
import { useMediaQuery } from "react-responsive";
import { Tab, Tabs } from "react-bootstrap";

const Enterpreneurship = () => {
  const [key, setKey] = useState("home");

  return (
    <>
      <div className="course-outlines mt">
        <div className="bg-img">
          <div className="container pt-5">
            <h1 className="fw-bold text-white">Courses</h1>
            <p className="mb-0 small fw-bold text-white">
              Home &bull; Available Programs &bull; Fast Track &bull;
              Enterpreneurship
            </p>
          </div>
        </div>

        <div className="container">
          <div className="row">
            <div className="col-lg-9 mt-4 mt-lg-5">
              <div className="d-flex justify-content-between align-items-center">
                <h2 className="fw-bold mb-0">Enterpreneurship</h2>

                <div>
                  <Button
                    // onClick={() => push("/apply-now")}
                    variant="contained"
                    fullWidth
                    className="custom-button mt-3"
                  >
                    Apply Now
                  </Button>
                </div>
              </div>

              <div className="my-3">
                <img
                  src="https://c1.wallpaperflare.com/preview/763/123/778/technology-lumia-mobile-smartphone.jpg"
                  alt=""
                  width="100%"
                  className="rounded "
                  height="400"
                />
              </div>

              <Tabs
                id="controlled-tab-example"
                activeKey={key}
                onSelect={(k) => setKey(k)}
              >
                <Tab eventKey="home" title="Overview">
                  <h3 className="fw-bold">Course Description</h3>
                  <p>
                    This course is intended for aspiring and struggling
                    entrepreneurs getting to help them off the ground. Course
                    content will focus on practical knowledge rather than
                    theoretical and from day one, with the help of assignments
                    and quizzes, a challenging environment will be created to
                    properly nurture the minds of students. The course will
                    include a Capstone project that will challenge the students
                    to test the skills they learned in this course.
                  </p>

                  <ul>
                    <li>
                      To provide hands on practical experience to trainees in
                      the field of Entrepreneurship
                    </li>
                    <li>
                      To prepare trainees for obstacles faced by new
                      entrepreneurs
                    </li>
                    <li>
                      To create the mindset of creative idea generation in
                      aspiring entrepreneurs
                    </li>
                    <li>To help building upon the idea</li>
                    <li>
                      To help entrepreneurs come up with key documents required
                    </li>
                    <li>
                      To create the mindset of capitalizing on the opportunities
                    </li>
                    <li>
                      To help aspiring entrepreneurs understand global market
                      trends
                    </li>
                    <li>
                      To help aspiring entrepreneurs write and deliver elevator
                      pitch
                    </li>
                    <li>To teach ethics and responsibility in business</li>
                  </ul>

                  <p>
                    On Successful completion of this course, students will have
                    developed the mindset required to be a successful
                    entrepreneur. After the completion of this course, each
                    student will have come up with their unique ideas of
                    entrepreneurship and have all completed all the necessary
                    documents needed to get started.
                  </p>
                </Tab>
                <Tab eventKey="requi" title="Prerequisite"></Tab>
                <Tab eventKey="profile" title="Curriculum">
                  <div className="accordion" id="accordionExample">
                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingOne">
                        <button
                          className="accordion-button"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#collapseOne"
                          aria-expanded="true"
                          aria-controls="collapseOne"
                        >
                          Week 1
                        </button>
                      </h2>
                      <div
                        id="collapseOne"
                        className="accordion-collapse collapse show"
                        aria-labelledby="headingOne"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 1
                              <ul>
                                <li>Course Introduction and Overview</li>
                                <li>What is Entrepreneurship</li>
                                <li>
                                  Difference between a businessman and an
                                  entrepreneur
                                </li>
                              </ul>
                            </li>
                            <li>
                              Lecture 2
                              <ul>
                                <li>Why choose entrepreneurship?</li>
                                <li>
                                  What does it take to be an Entrepreneur?
                                </li>
                                <li>Traits of a successful Entrepreneur</li>
                                <li>Different Types of Business structures</li>
                              </ul>
                            </li>
                            <li>Quiz 1</li>
                          </ul>
                        </div>
                      </div>
                    </div>
                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingTwo">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#collapseTwo"
                          aria-expanded="false"
                          aria-controls="collapseTwo"
                        >
                          Week 2
                        </button>
                      </h2>
                      <div
                        id="collapseTwo"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingTwo"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 3
                              <ul>
                                <li>What is an idea?</li>
                                <li>How to come up with an idea?</li>
                                <li>
                                  Metrics to consider when thinking of an idea
                                </li>
                                <li>Outlining your ideas</li>
                              </ul>
                            </li>
                            <li>Assignment 1</li>
                            <li>
                              Lecture 4
                              <ul>
                                <li>How to assess and test your idea?</li>
                                <li>
                                  Role of market trends when finalizing your
                                  idea
                                </li>
                                <li>
                                  Identifying your Target Market and Customers
                                </li>
                              </ul>
                            </li>
                            <li>Quiz 2</li>
                          </ul>
                        </div>
                      </div>
                    </div>
                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#collapseThree"
                          aria-expanded="false"
                          aria-controls="collapseThree"
                        >
                          Week 3
                        </button>
                      </h2>
                      <div
                        id="collapseThree"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 5
                              <ul>
                                <li>Understanding different business models</li>
                                <li>
                                  Choosing the right business model for your
                                  idea
                                </li>
                                <li>How to be different from the rest</li>
                              </ul>
                            </li>

                            <li>
                              Lecture 6
                              <ul>
                                <li>What is an Elevator Pitch?</li>
                                <li>Writing an Elevator Pitch for your idea</li>
                                <li>How to deliver an Elevator Pitch</li>
                              </ul>
                            </li>
                            <li>Assignment 2</li>
                            <li>Quiz 3</li>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m1"
                          aria-expanded="false"
                          aria-controls="m1"
                        >
                          Week 4
                        </button>
                      </h2>
                      <div
                        id="m1"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <ul>
                              <li>
                                Lecture 7
                                <ul>
                                  <li>What is a Business Plan?</li>
                                  <li>Making a business plan for your idea</li>
                                  <li>
                                    Metrics to consider before writing a
                                    business plan
                                  </li>
                                </ul>
                              </li>
                              <li>Assignment 3</li>
                              <li>
                                Lecture 8
                                <ul>
                                  <li>Business Communication</li>
                                  <li>Funding Options</li>
                                  <li>How to approach investors</li>
                                  <li>Different models of investment</li>
                                </ul>
                              </li>

                              <li>Quiz 4</li>
                            </ul>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m2"
                          aria-expanded="false"
                          aria-controls="m2"
                        >
                          Week 5
                        </button>
                      </h2>
                      <div
                        id="m2"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <ul>
                              <li>
                                Lecture 9
                                <ul>
                                  <li>Introduction to Marketing</li>
                                  <li>Introduction to Branding</li>
                                  <li>Creating a brand book</li>
                                  <li>
                                    Do’s and Don'ts of Marketing and Branding
                                  </li>
                                  <li>Creating your marketing strategy</li>
                                </ul>
                              </li>

                              <li>
                                Lecture 10
                                <ul>
                                  <li>
                                    Introduction to Social Media Promotion
                                  </li>
                                  <li>Leveraging Social Media</li>
                                  <li>
                                    Demographics and psycho-graphics research
                                  </li>
                                  <li>Audience Segmentation</li>
                                  <li>Digital Marketing</li>
                                </ul>
                              </li>

                              <li>Quiz 5</li>
                            </ul>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m3"
                          aria-expanded="false"
                          aria-controls="m3"
                        >
                          Week 6
                        </button>
                      </h2>
                      <div
                        id="m3"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <ul>
                              <li>
                                Lecture 11
                                <ul>
                                  <li>
                                    Further on Social Media Leveraging and
                                    Digital Marketing
                                  </li>
                                  <li>
                                    Marketing Personas Development for Social
                                    Media Marketing
                                  </li>
                                  <li>Introduction to Paid Promotions</li>
                                </ul>
                              </li>

                              <li>
                                Lecture 12
                                <ul>
                                  <li>Further on Paid Promotions</li>
                                  <li>Getting Creative!</li>
                                  <li>Role of Graphic layouts in promotions</li>
                                  <li>Do’s and Don'ts of Paid Promotion</li>
                                </ul>
                              </li>

                              <li>Quiz 6</li>
                            </ul>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m4"
                          aria-expanded="false"
                          aria-controls="m4"
                        >
                          Week 7
                        </button>
                      </h2>
                      <div
                        id="m4"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <ul>
                              <li>
                                Lecture 13
                                <ul>
                                  <li>What is Value addition?</li>
                                  <li>Adding value to your products</li>
                                  <li>Communicating value to your customers</li>
                                </ul>
                              </li>

                              <li>
                                Lecture 14
                                <ul>
                                  <li>Getting off the ground</li>
                                  <li>Do you need an office?</li>
                                  <li>Office based vs home based businesses</li>
                                  <li>
                                    Traditional vs Paperless business models
                                  </li>
                                </ul>
                              </li>

                              <li>Quiz 7</li>
                            </ul>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m5"
                          aria-expanded="false"
                          aria-controls="m5"
                        >
                          Week 8
                        </button>
                      </h2>
                      <div
                        id="m5"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <ul>
                              <li>
                                Lecture 15
                                <ul>
                                  <li>When to make your first hire?</li>
                                  <li>
                                    How to find the right talent for your
                                    business
                                  </li>
                                  <li>Engaging the right people</li>
                                </ul>
                              </li>

                              <li>
                                Lecture 16
                                <ul>
                                  <li>Keeping the business moving</li>
                                  <li>
                                    Common struggles and how to address them
                                  </li>
                                  <li>Organizational Behavior</li>
                                </ul>
                              </li>

                              <li>Quiz 8</li>
                            </ul>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m6"
                          aria-expanded="false"
                          aria-controls="m6"
                        >
                          Week 9
                        </button>
                      </h2>
                      <div
                        id="m6"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <ul>
                              <li>
                                Lecture 17
                                <ul>
                                  <li>How to scale your business?</li>
                                  <li>
                                    Common struggles in scaling and how to
                                    address them
                                  </li>
                                </ul>
                              </li>

                              <li>
                                Lecture 18
                                <ul>
                                  <li>
                                    Why most businesses fail in the long run
                                  </li>
                                  <li>
                                    Sustainability and control over your
                                    business
                                  </li>
                                  <li>Adapting to the trends</li>
                                </ul>
                              </li>

                              <li>Quiz 9</li>
                            </ul>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m7"
                          aria-expanded="false"
                          aria-controls="m7"
                        >
                          Week 10
                        </button>
                      </h2>
                      <div
                        id="m7"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 19
                              <ul>
                                <li>
                                  Introduction to Financial Statements and
                                  Accounting
                                </li>
                                <li>
                                  Ledger, Balance Sheet, Income Statement & Cash
                                  flows statement
                                </li>
                              </ul>
                            </li>

                            <li>
                              Lecture 20
                              <ul>
                                <li>Further on Financial Statements</li>
                                <li>
                                  Why maintain proper financial statements
                                </li>
                              </ul>
                            </li>

                            <li>Quiz 10</li>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m8"
                          aria-expanded="false"
                          aria-controls="m8"
                        >
                          Week 11
                        </button>
                      </h2>
                      <div
                        id="m8"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 21
                              <ul>
                                <li>Introduction to Sales</li>
                                <li>What is a sales pipelines</li>
                                <li>Steps from Prospect to a sale</li>
                                <li>Basic Sales Training</li>
                              </ul>
                            </li>

                            <li>
                              Lecture 22
                              <ul>
                                <li>Further on Selling</li>
                                <li>Writing a sales pitch that doesn’t fail</li>
                                <li>Sales conversion online</li>
                                <li>Sales conversion in person</li>
                                <li>Advanced Sales Training</li>
                              </ul>
                            </li>
                            <li>Assignment 5</li>
                            <li>Quiz 11</li>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m9"
                          aria-expanded="false"
                          aria-controls="m9"
                        >
                          Week 12
                        </button>
                      </h2>
                      <div
                        id="m9"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 23
                              <ul>
                                <li>Globalization</li>
                                <li>How to benefit from Globalization</li>
                                <li>Going global from Pakistan</li>
                              </ul>
                            </li>

                            <li>
                              Lecture 24
                              <ul>
                                <li>Customer Experience</li>
                                <li>How to retain clients?</li>
                                <li>
                                  Developing strategies to benefit existing
                                  clients
                                </li>
                              </ul>
                            </li>

                            <li>Quiz 12</li>
                          </ul>
                        </div>
                      </div>
                    </div>

                    <div className="accordion-item">
                      <h2 className="accordion-header" id="headingThree">
                        <button
                          className="accordion-button collapsed"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#m10"
                          aria-expanded="false"
                          aria-controls="m10"
                        >
                          Week 13
                        </button>
                      </h2>
                      <div
                        id="m10"
                        className="accordion-collapse collapse"
                        aria-labelledby="headingThree"
                        data-bs-parent="#accordionExample"
                      >
                        <div className="accordion-body">
                          <ul>
                            <li>
                              Lecture 25
                              <ul>
                                <li>
                                  Capstone Project Introduction and Guidelines{" "}
                                </li>
                              </ul>
                            </li>
                          </ul>
                        </div>
                      </div>
                    </div>
                  </div>
                </Tab>
                <Tab eventKey="contact" title="Instructor">
                  <div className="d-flex flex-sm-row flex-column px-3 justify-content-center align-items-center mb-5">
                    <img
                      src="data:image/png;base64,"
                      alt=""
                      width="200"
                      height="200"
                    />

                    <div className="ms-3">
                      <h5 className="fw-bold">Name</h5>
                      <p>Instructor</p>
                      <p className="mb-0">
                        Lorem ipsum dolor sit amet consectetur adipisicing elit.
                        Laudantium maiores laborum laboriosam placeat vitae quae
                        blanditiis facilis impedit! Placeat nihil temporibus
                        magnam est autem quis ex aperiam inventore, voluptatem
                        quae!
                      </p>
                    </div>
                  </div>
                </Tab>
              </Tabs>

              <h3 className="fw-bold mb-0">Related</h3>
              <div className="row">
                <div className="col-sm-6 col-md-4 mt-4">
                  <div className="card w-100">
                    <img
                      src="https://i.ibb.co/3pV01zc/ASP-NET-Web-Applications.jpg"
                      className="card-img-top w-100"
                      style={{ borderRadius: 0, height: 130 }}
                      alt="..."
                    />
                    <div className="card-body d-flex flex-column align-items-center">
                      <div className="d-flex flex-column align-items-center img-par">
                        <div className="image d-flex flex-column align-items-center">
                          <img
                            src="https://i.ibb.co/CVCp97m/FARUKH-KHAN.png"
                            alt=""
                            width="60"
                          />
                        </div>
                        <h6 className="mt-2">FARUKH KHAN</h6>
                      </div>

                      <h5 className="card-title text-center my-card-title-font ">
                        ASP.NET Web Applications
                      </h5>
                    </div>
                  </div>
                </div>
                <div className="col-sm-6 col-md-4 mt-4">
                  <div className="card w-100">
                    <img
                      src="https://i.ibb.co/KmnkbPp/PHP-Laravel.jpg"
                      className="card-img-top w-100"
                      style={{ borderRadius: 0, height: 130 }}
                      alt="..."
                    />
                    <div className="card-body d-flex flex-column align-items-center">
                      <div className="d-flex flex-column align-items-center img-par">
                        <div className="image d-flex flex-column align-items-center">
                          <img
                            src="https://i.ibb.co/7gfCGj5/ABDUL-BASSIT-SURAHIO.png"
                            alt=""
                            width="60"
                          />
                        </div>
                        <h6 className="mt-2 ">Name</h6>
                      </div>

                      <h5 className="card-title text-center my-card-title-font ">
                        PHP Laravel
                      </h5>
                    </div>
                  </div>
                </div>
                <div className="col-sm-6 col-md-4 mt-4">
                  <div className="card w-100">
                    <img
                      src="https://i.ibb.co/vmWqn6k/MERN-Stack.jpg"
                      className="card-img-top w-100"
                      style={{ borderRadius: 0, height: 130 }}
                      alt="..."
                    />
                    <div className="card-body d-flex flex-column align-items-center">
                      <div className="d-flex flex-column align-items-center img-par">
                        <div className="image d-flex flex-column align-items-center">
                          <img
                            src="https://i.ibb.co/JBRprYC/WAQAS-QAMAR.png"
                            alt=""
                            width="60"
                          />
                        </div>
                        <h6 className="mt-2">Name</h6>
                      </div>

                      <h5 className="card-title text-center my-card-title-font ">
                        MERN Stack
                      </h5>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-lg-3 mt-4 mt-lg-5">
              <Paper elevation={10} className="p-3 border">
                <h6 className="fw-bold mb-0 details text-center text-sm-start">
                  Instructor: <span className="text-success"></span>
                </h6>
                <h6 className="fw-bold mb-0 details text-center text-sm-start">
                  Lectures: <span className="fw-normal">26</span>
                </h6>
                <h6 className="fw-bold mb-0 details text-center text-sm-start">
                  Duration: <span className="fw-normal">13 Weeks</span>
                </h6>
                <h6 className="fw-bold mb-0 details text-center text-sm-start">
                  Enrolled: <span className="fw-normal">0 Students</span>
                </h6>
                <h6 className="fw-bold mb-0 details text-center text-sm-start">
                  Language: <span className="fw-normal">Urdu / English</span>
                </h6>
                <Button
                  // onClick={() => push("/apply-now")}
                  variant="contained"
                  fullWidth
                  className="custom-button mt-3"
                >
                  Enroll Now
                </Button>
              </Paper>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Enterpreneurship;
